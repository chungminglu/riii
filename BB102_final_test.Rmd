...
g2 = g+ geom_histogram(aes(price_per_sqmeter))+facet_wrap(~area)
g2
...
house=merge(x=house,y=house_danger,all.x=T)
house
```
...
variable_list=names(house)%in%c('area','building_age','building_sqmeter','building_type','price_per_sqmeter','danger')
house=house[,variable_list]
set.seed(1206)
ind<-sample(1:2, size=nrow(house), replace=T, prob=c(0.8, 0.2))
trainset=house[ind==1,]
testset=house[ind==2,]
...
table(testset$danger, predictions)
```
